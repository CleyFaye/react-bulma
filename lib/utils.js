/*eslint-env commonjs */
let dummyNameId = 0;

// TODO Make all the functions const
// TODO Remove env commonjs
// TODO Fix formatting
// TODO Use filterClass for other functions in this file
// TODO Replace push.apply() with '...customClasses'

/** Add content of customClasses into classes
 *
 * @param {Array.<{string}>} classes
 * Initial list of classes
 *
 * @param {string|Array.<{string}>} customClasses
 * List of classes to add
 *
 * @returns {Array.<{string}>}
 * The same array as classes
 */
export const addCustomClasses = (classes, customClasses) => {
  if (!Array.isArray(customClasses)) {
    customClasses = [customClasses];
  }
  Array.prototype.push.apply(classes, customClasses);
  return classes;
};

/** Return a unique dummy name */
export let getDummyName = () => `dummy${dummyNameId++}`;

/** Filter a value to match a list of candidates */
const filterClass = (value, candidates, prefix, suffix) => {
  if (candidates.includes(value)) {
    return `${prefix}${value}${suffix}`;
  }
  throw new Error(`Invalid value "${value}" (must be in ${candidates})`);
};

/** Convert a breakpoint level to a bulma class.
 *
 * @param {string} breakpoint
 * @returns {string}
 */
export const breakpointToClass = breakpoint => 
  filterClass(
    breakpoint,
    ["mobile", "tablet", "desktop", "widescreen", "fullhd"],
    "is-");

/** Convert a color name to a bulma class.
 *
 * @param {string} colorName
 * @returns {string}
 */
export let colorToClass = colorName => {
    switch (colorName) {
    case "primary":
    case "link":
    case "info":
    case "success":
    case "warning":
    case "danger":
    case "white":
    case "light":
    case "dark":
    case "black":
        return `is-${colorName}`;
    }
    throw new Error(`Unknown color name "${colorName}"`);
};

/** Convert a size name to a CSS class
 * @param {string} sizeName
 * @returns {string}
 */
export let sizeToClass = sizeName => {
    switch (sizeName) {
    case "small":
    case "normal":
    case "medium":
    case "large":
        return `is-${sizeName}`;
    }
    throw new Error(`Unknown size name "${sizeName}"`);
};

/** Convert a width (in column) to a CSS class
 * @param {string|number} width
 * @returns {string}
 */
export const widthToClass = width => {
  const convertedWidth = parseInt(width);
  if (convertedWidth >= 1 && convertedWidth <= 12) {
    return `is-${convertedWidth}`;
  }
  throw new Error(`Unknown column width "${width}"`);
};

/** Add common modifier classes to an array of classes
 * @param {object} props
 * @param {Array.<{string}>} classes
 * List of classes to update
 * @param {object} options
 * Options to ignore some values. Accepted properties:
 * - ignoreClassName: Don't check className
 * - ignoreFullWidth: Don't check "fullwidth" prop
 * - ignoreSize: Don't check "size" prop
 * - ignoreWidth: Don't check "width" prop
 * - ignoreColor: Don't check "color" prop
 * - ignoreInverted: Don't check "inverted" prop
 * - ignoreLoading: Don't check "loading" prop
 */
export let commonModifiers = (props, classes, options) => {
    if (!options) {
        options = {};
    }
  if (!options.ignoreClassName && props.className) {
    addCustomClasses(classes, props.className);
  }
    if (!options.ignoreFullWidth && props.fullwidth) {
        classes.push("is-fullwidth");
    }
    if (!options.ignoreSize && props.size) {
        classes.push(sizeToClass(props.size));
    }
  if (!options.ignoreWidth && props.width) {
    classes.push(widthToClass(props.width));
  }
    if (!options.ignoreColor && props.color) {
        classes.push(colorToClass(props.color));
    }
    if (!options.ignoreInverted && props.inverted) {
        classes.push("is-inverted");
    }
    if (!options.ignoreLoading && props.loading) {
        classes.push("is-loading");
    }
};
