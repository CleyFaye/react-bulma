...
exports.addClassesWithViewportSuffix = exports.addClassesFromOptions = exports.addClassFromList = exports.classString = exports.addBreakpoint = void 0;
 ...
 * Return classes
 */
const addBreakpoint = (classes, breakpoint) => {
  if (!breakpoint) {
    return classes;
  }

  if (!_const.breakpointList.includes(breakpoint)) {
...
const classString = (classes, extraClassName) => {
  if (!classes) {
    classes = [];
  }
  ...
  return classesSet.size == 0 ? undefined : Array.from(classesSet.values()).join(" ");
};
...
const addClassFromList = (classes, property, list, transform) => {
  if (typeof property !== "undefined") {
    if (!list.includes(property)) {
      throw new Error(`Unexpected value "${property}" ` + `(must be ${list.map(elem => `"${elem}"`).join(",")})`);
    }

    classes.push(transform ? transform(property) : `is-${property}`);
  }
...
exports.addClassFromList = addClassFromList;

const addClassesFromOptions = (classes, propList, options, supportedOptions) => Object.keys(supportedOptions).reduce((classes, supportedOptionName) => {
  const option = options ? options[supportedOptionName] : true;
  ...
  return addClassFromList(classes, propList[typeof option === "string" ? option : supportedOptionName], supportedOption.list, supportedOption.transform);
}, classes);
...
exports.addClassesFromOptions = addClassesFromOptions;

const addClassesWithViewportSuffix = (classes, values, transform, onlyBreakpoint) => {
  if (!values) {
    return classes;
  }

  const acceptableList = onlyBreakpoint ? _const.breakpointList : _const.viewportList;
...
exports.addClassesWithViewportSuffix = addClassesWithViewportSuffix;
